<template>
 ...
 <div class="container">
   ...
   <div class="content"  v-if="!uiProps.gameOver">
     <div class="players" v-if="!uiProps.gameStarted">
       <div class="mb">
         <h5>Player One</h5>
         <input type="text" v-model="uiProps.playerOne.name" data-playerOne>
       </div>
       <div class="mb">
         <h5>Player Two</h5>
         <input type="text" v-model="uiProps.playerTwo.name" data-playerTwo>
       </div>
       <button @click="startGame" id="startGame">Start</button>
     </div>
...
if(this.nextQuestion){
  ...
  if(valid){
    var conf = confirm("Still in the game?");
    if(conf){
  ...
  computed: {
    ...
    nextQuestion(){ 
      ...
      if(!cq){
        ...
        if(cq.answers.length === this.uiProps.maxAnswersCount){
          var conf = confirm("Still in the game?");
          if(conf){
            question = null;
            this.clearQuestion();
