...
this.uiProps.questions[this.nextQuestion.questionKey].answers.push(GameModels.answer)
  ...
  validateAnswer(){
    if(this.uiProps.questions[this.nextQuestion.questionKey].value === this.uiProps.answer){
      alert(`Correct Answer!!!`)
    }else{
      alert(`Wrong guess!!! You have ${this.guessCount} tries left`)
    }
  }
  },
  computed: {
    ...
    nextQuestion(){ 
      ...
      this.uiProps.questions.forEach( (item, i)=>{
        if(item.answers.length === this.uiProps.maxAnswersCount){
          question = this.uiProps.questions[i+1];
        }else{
          question = item;
        }
      })
